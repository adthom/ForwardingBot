...
namespace ForwardingBot.Common.Authentication
{
    using System;
    ...
    public class AuthenticationProvider : ObjectRoot, IRequestAuthenticationProvider
    {
        ...
        public async Task AuthenticateOutboundRequestAsync(HttpRequestMessage request, string tenant)
        {
            ...
            tenant = string.IsNullOrWhiteSpace(tenant) ? "organizations" : tenant;
            ...
            try
            {
                ...
                result = await this.AcquireTokenWithRetryAsync(app, resource, attempts: 3).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        /// </returns>
        private async Task<AuthenticationResult> AcquireTokenWithRetryAsync(IConfidentialClientApplication app, string resource, int attempts)
        {
            while (true)
            {
                attempts--;
                ...
                try
                {
                    return await app.AcquireTokenForClient(new[] { $"{resource}/.default" }).ExecuteAsync().ConfigureAwait(false);
                }
                catch (Exception) when (attempts >= 1)
                {
                }
